...
tcp_packet *recvpkt;
tcp_packet *b_packet[MAX_WINDOW];
tcp_packet *sndpkt;
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        while(1)
        {
            ...
            //VLOG(DEBUG,"received pkt seqno; %d expt_seq:%d",recvpkt->hdr.seqno,expt_seq);   
           ...
           // VLOG(DEBUG,"temp packet received of seqno %d",recvpkt->hdr.seqno);
            ...
            else if(recvpkt->hdr.seqno>expt_seq && b_index<MAX_WINDOW)
            {
        ...
        VLOG(DEBUG, "%lu.%lu,%d,%d",tp2.tv_sec,tp2.tv_usec*1000,recvpkt->hdr.data_size, recvpkt->hdr.seqno);
